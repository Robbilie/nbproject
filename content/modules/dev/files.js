...
"</table><br/><div><i>Once you've created a class, you can add files to it and invite users...</i></div></div>")
  ...
  // Returns 'file', 'html', 'youtube' or 'html5video' depending on the selected upload type ('' if there is an error)
  function getUploadType() {
    ...
    if (document.getElementById('html5video_select').checked) {return 'html5video';}
  ...
  function updateVisibility(e) {
    if (getUploadType() === 'file') {
        ...
        $("#add_html5video_div").css("display", "none");
    }
    ...
    if (getUploadType() === 'html5video') {
        $("#add_file_div").css("display", "none");
        $("#add_html_div").css("display", "none");
        $("#add_youtube_div").css("display", "none");
        $("#add_html5video_div").css("display", "inline");
    }
  }
  ...
  GLOB.files.addFile = function (id_ensemble, id_folder) {
    ...
    $('#add_file_html5video').attr('href', '/addhtml5video/' + GLOB.files.currentEnsemble);
  ...
  GLOB.files.proceedUpload = function (payload) {
    ...
    if (getUploadType() === 'html') {
      ...
      $('#youtube_upload_form')
          ...
          .submit()[0]
          .reset();
    } else if (getUploadType() === 'html5video') {
      console.log("html5video Upload");
      $('#html5video_upload_form')
          .attr('action', '/addhtml5video/' + GLOB.files.currentEnsemble)
          .submit()[0]
          .reset();
    } else {
